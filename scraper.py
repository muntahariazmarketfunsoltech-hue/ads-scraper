...
# =========================
# PACKAGE MATCHING (REFRESHED & OPTIMIZED)
# =========================
...
def score_package_against_text(pkg, headline, description):
    """
    STRICT scoring: compare package with visible headline + description.
    ...
    # STRONGEST SIGNAL: package core appears directly in visible ad text
    ...
    for token in tokens:
        ...
        # Allow long tokens to match joined visible text
    ...
    # TWO OR MORE EXACT HITS = HIGH CONFIDENCE
    if len(exact_hits) >= 2:
        score = max(score, 0.92)
    # ONE EXACT HIT WITH 8+ CHARS = MEDIUM-HIGH CONFIDENCE
    elif len(exact_hits) == 1 and len(exact_hits[0]) >= 8:
        score = max(score, 0.80)
    # TWO OR MORE PARTIAL HITS = MEDIUM CONFIDENCE
    elif total_hits >= 2:
        score = max(score, 0.77)
    # FUZZY CORE MATCHING (only if core is 8+ chars and very similar)
...
def get_best_matching_package(headline, description, package_list, min_score=MIN_PACKAGE_MATCH_SCORE):
    ...
    Returns (package, score). If no match >= min_score, returns (None, best_score).
    """
    if not package_list:
        return None, 0.0
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if visible_package != "N/A":
                ...
                if has_text:
                    print(f"📦 Row {row_num}: matching headline+description with packages...")
                ...
                if package_name:
                    ...
                    status = "SUCCESS"
                    message = f"Non-video {ad_type} matched with score {match_score}"
                    print(f"✅ Row {row_num}: matched package -> {package_name} | score={match_score}")
                else:
                    package_name = "N/A"
                    app_link = "N/A"
                    status = "NON_VIDEO_PACKAGE_NOT_FOUND"
                    message = f"Non-video {ad_type} package score {match_score} < 0.76"
                    print(f"⚠️ Row {row_num}: score {match_score} < 0.76, N/A written")
