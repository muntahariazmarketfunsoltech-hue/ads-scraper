...
# FULL COMPLETE CODE - STABLE PACKAGE MATCHING LOCKED AT 0.76
...
# ========================================
MIN_PACKAGE_MATCH_SCORE = 0.76  # FROZEN THRESHOLD
EXACT_HITS_HIGH = 0.92           # TWO+ EXACT HITS
EXACT_HITS_LONG = 0.80           # ONE 8+ CHAR EXACT HIT
PARTIAL_HITS_MIN = 0.77          # TWO+ PARTIAL HITS
CORE_FUZZY_MIN = 0.82            # FUZZY CORE MATCH (8+ chars, 88%+ similar)
DIRECT_CORE_FOUND = 0.98         # PACKAGE CORE DIRECTLY IN TEXT (6+ chars)
...
# =========================
# VIDEO ID LOGIC (ORIGINAL - UNCHANGED)
# =========================

def is_real_video_response(response):
    """Check if response is a real video file."""
    try:
...
def extract_video_id_from_url(req_url):
    """Extracts clean video IDs or filenames."""
    try:
...
def scan_browser_performance_for_video(page):
    """Scans performance entries for real video URLs."""
    try:
        urls = page.evaluate("""
...
def wait_for_video_id(page, captured, max_seconds=20):
    """Waits for video ID to be captured."""
    waited = 0
...
def clean_googleadservices_link(href):
    """Cleans up Google Ads Services redirect links."""
    if not href:
        return "N/A"
...
def is_good_app_link(href):
    """Checks if link is a valid app store link."""
    if not href:
        return False
...
def get_visible_install_candidates_from_target(target):
    """Gets install button candidates from page/frame."""
    candidates = []
...
def extract_install_link_by_precise_js(page):
    """Strict JS fallback for install links."""
    js = r"""
    () => {
...
def wait_and_extract_install_link(page, max_wait_seconds=35):
    """Waits and extracts install link from page."""
    start = time.time()
...
# =========================
# STABLE PACKAGE MATCHING (LOCKED)
# =========================
...
def score_package_against_text_STABLE(pkg, headline, description):
    """
    LOCKED STABLE SCORING for package matching.
    All thresholds are frozen constants.
    Returns score 0.0 to 1.0 based on FIXED rules.
    """
    ...
    # RULE 1: DIRECT CORE MATCH (6+ chars in visible text)
    if package_core and len(package_core) >= 6 and package_core in visible_clean:
        score = max(score, DIRECT_CORE_FOUND)  # 0.98

    # RULE 2: EXACT TOKEN MATCHES
    exact_hits = []
    partial_hits = []
    ...
    for token in tokens:
        ...
        # Long tokens matching joined text
    ...
    # RULE 3: TWO+ EXACT HITS = HIGH CONFIDENCE (FROZEN)
    if len(exact_hits) >= 2:
        score = max(score, EXACT_HITS_HIGH)  # 0.92

    # RULE 4: ONE EXACT HIT (8+ chars) = MEDIUM-HIGH (FROZEN)
    elif len(exact_hits) == 1 and len(exact_hits[0]) >= 8:
        score = max(score, EXACT_HITS_LONG)  # 0.80

    # RULE 5: TWO+ PARTIAL HITS = MEDIUM (FROZEN)
    elif total_hits >= 2:
        score = max(score, PARTIAL_HITS_MIN)  # 0.77

    # RULE 6: FUZZY CORE MATCHING (ONLY IF CORE 8+ chars AND 88%+ similar)
    elif package_core and len(package_core) >= 8:
        core_ratio = difflib.SequenceMatcher(None, visible_clean, package_core).ratio()
        if core_ratio >= 0.88:
            score = max(score, CORE_FUZZY_MIN)  # 0.82

    return round(score, 4)


def get_best_matching_package_STABLE(headline, description, package_list):
    """
    LOCKED package matching function.
    Uses FROZEN MIN_PACKAGE_MATCH_SCORE threshold.
    ...
    for pkg in sorted(package_list):
        score = score_package_against_text_STABLE(pkg, headline, description)
        if score > best_score:
            best_score = score
            best_pkg = pkg

    # FROZEN threshold: 0.76 - never changes
...
def extract_packages_from_text(raw_text):
    """Returns a SET of all unique, valid package names found in text."""
    ...
    patterns = [
        r"""['\"](appId)['\"]\s*:\s*['\"]([
 A-Za-z][\w.]+)['\"]""",
        r"""play\.google\.com/store/apps/details[^\s'\"<>]*[?&]id=([
A-Za-z][A-Za-z0-9_]*(?:\.[A-Za-z][A-Za-z0-9_]*){2,})""",
        r"""market://[^\s'\"]*[?&]id=([
A-Za-z][A-Za-z0-9_]*(?:\.[A-Za-z][A-Za-z0-9_]*){2,})""",
        r"""(?:destination_url|final_url|click_url|destUrl|clickUrl|landingUrl)['\"]\s*:['\"]\s*['\"](^'\"]*[?&]id=([
A-Za-z][A-Za-z0-9_]*(?:\.[A-Za-z][A-Za-z0-9_]*){2,})""",
        r"""[?&]id=([
A-Za-z][A-Za-z0-9_]*(?:\.[A-Za-z][A-Za-z0-9_]*){2,})""",
        r"""[?&]package=([
...
def is_valid_text_ad(headline, description):
    """Checks if headline/description is valid."""
...
def scrape_single_url(url_row):
    """Scrapes single transparency URL for video/text/image ads."""
    row_num, url = url_row
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if has_text:
                print(f"🔎 Row {row_num}: text/image headline -> {headline[:50]}...")

            # PACKAGE RESOLUTION WITH STABLE MATCHING
            if visible_package != "N/A":
                ...
                if has_text:
                    print(f"📦 Row {row_num}: STABLE matching headline+description with packages (threshold={MIN_PACKAGE_MATCH_SCORE})...")
                    all_found_packages = extract_package_from_page(page)
                    package_name, match_score = get_best_matching_package_STABLE(headline, description, all_found_packages)
                ...
                if package_name:
                    ...
                    print(f"⚠️ Row {row_num}: score {match_score} < {MIN_PACKAGE_MATCH_SCORE}, N/A written")
...
def run_parallel_combined_scraper(max_workers=MAX_WORKERS):
    """Main entry point - runs parallel scraping with locked matching."""
    ...
    print(f"🔒 PACKAGE MATCHING LOCKED: min_score={MIN_PACKAGE_MATCH_SCORE} (FROZEN)")
    print(f"   - EXACT_HITS_HIGH: {EXACT_HITS_HIGH}")
    print(f"   - EXACT_HITS_LONG: {EXACT_HITS_LONG}")
    print(f"   - PARTIAL_HITS_MIN: {PARTIAL_HITS_MIN}")
    print(f"   - CORE_FUZZY_MIN: {CORE_FUZZY_MIN}")
    print(f"   - DIRECT_CORE_FOUND: {DIRECT_CORE_FOUND}")
